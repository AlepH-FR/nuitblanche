...
class GDCharter implements CharterInterface
{
	...
	public function populate()
	{
		...
		// data values
		self::imagewrite($frame, 10, 8, $this->height - 0, "0", $this->colors['black']);
		self::imagewrite($frame, 10, 0, ($this->height / 2) + 5, floor($this->max_apm / 2), $this->colors['black']);
		self::imagewrite($frame, 10, 0, 10, floor($this->max_apm), $this->colors['black']);

		$length_minutes = ($this->replay->getLength() / 60);
		for ($i = 0; $i < $length_minutes; $i+=5)
		{
			self::imagewrite($frame, 10, 25 + ($this->width / ($length_minutes / 5) * ($i / 5)), $this->height + 12, $i . " min", $this->colors['black']);
			if ($i > 0)
			{
				imageline(
					$frame,
