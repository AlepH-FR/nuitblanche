...
class SC2Chart
{
  ...
  public function _parsePlayer($player_name) 
  {
  ...
  public function _populate()
  {
    // creating image
  	...
  	foreach($this->_plots as $player_name => $plots)
  	{
    	...
    	{	
    		...
    		imageline(
    		  $pic,
    		  $i - self::PRECISION + 1, self::CHART_HEIGTH - $plots[$i - self::PRECISION] / $max_apm * self::CHART_HEIGTH, 
    		  $i, self::CHART_HEIGTH - $plots[$i] / $max_apm * self::CHART_HEIGTH,
    		  $colors[$player_name]
    		);
    	}
    ...
    // data values
	  ...
	  for ($i = 0; $i < $length_minutes; $i+=5)
  	{
  		...
  		if ($i > 0)
  		{
  			imageline($frame, 25 + (self::CHART_WIDTH / ($length_minutes / 5) * ($i / 5)), 0, 25 + (self::CHART_WIDTH / ($length_minutes / 5) * ($i / 5)), self::CHART_HEIGTH, $gray);
  		}		
  	}
  ...
  public static function imagewrite($imgh, $size, $width, $height, $text, $color, $bold = false)
  {
